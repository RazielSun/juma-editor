...
from juma.core.Command 	import EditorCommandRegistry

from juma.qt          	import QtEditorModule
from juma.qt.helpers   	import addWidgetWithLayout, QColorF, unpackQColor
from juma.qt.IconCache                  import getIcon
from juma.qt.controls.GenericListWidget import GenericListWidget

from juma.MainEditor.MainEditor    		import MainEditorModule, getSceneSelectionManager
...
##----------------------------------------------------------------##
class CommandHistoryView( MainEditorModule ):
	_name = 'command_history_view'
	_dependency = [ 'main_editor' ]
	...
	def onStart( self ):
		...
		self.targetStack = registry.getCommandStack( 'main_editor' )
	...
	def onMoaiReady( self ):
		# todo: clear when moai reload
		self.listHistory.clear()

##----------------------------------------------------------------##

CommandHistoryView().register()
